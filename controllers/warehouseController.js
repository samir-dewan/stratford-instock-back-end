...
const listAllWarehouses = (_req, res) => {
  ...
  const listWarehouseAndInventoryById = (_req, res) => { 
	...
	let inventory = inventories.getInventoryByWarehouseId(_req.params.warehouseId);
	let newWarehouse = warehouses.concat(inventory);
	res.status(200).json(newWarehouse)}
	catch{
	  res.status(400).json({error:`Warehouse with ID ${_req.params.warehouseId} not found`})
	}
  }
